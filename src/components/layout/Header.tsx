  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || isMobileMenuOpen ? "glass-effect shadow-lg py-4" : "bg-transparent py-6"
      }`}
    >
          ...
          <nav className="md:hidden mt-6 flex flex-col gap-4 pb-4">
            {menuItems.map((item) => (
              <Link
                ...
                className={`bg-secondary/50font-mono text-lg transition-colors text-left px-4 py-2 rounded-lg ${
                  isActive(item.href)
